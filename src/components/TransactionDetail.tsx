...
export default function TransactionDetail({ transaction, userRole = 'viewer' }: { transaction: Transaction, userRole?: string }) {
  ...
  const downloadPDF = () => {
    ...
    // Header
    doc.setFontSize(22)
    doc.text("Admaniacs Finance", 20, 20)
    ...
    doc.text("Generated by Admaniacs Finance System", 20, 280)
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 p-6 md:p-12 font-sans relative">
      ...
      <div className="max-w-3xl mx-auto space-y-8">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden shadow-2xl relative">
          <div className="p-8 md:p-12 space-y-8">
            {/* Header */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-200 shadow-sm pb-8">
              <div>
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-gray-900 max-w-lg break-words">{transaction.title}</h1>
                <p className="text-gray-500 mt-2">{format(new Date(transaction.date), 'EEEE, MMMM do, yyyy')}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              ...
              {transaction.client?.name && (
                <div>
                  <p className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-2">Client</p>
                  <div className="flex items-center text-lg text-white">
                    <span className="w-2 h-2 rounded-full bg-blue-500 mr-2"></span>
                    {transaction.client.name}
                  </div>
            ...
            {transaction.customFields && transaction.customFields.length > 0 && (
                <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6">
                    <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-4">Additional Details</h3>
            ...
            {/* Audit Trail */}
            <div className="bg-white border border-gray-200 shadow-sm rounded-xl p-6">
                <h3 className="text-sm font-medium text-gray-500 uppercase tracking-wider mb-4">Activity Log</h3>
                <div className="space-y-3">
                    ...
                    {transaction.lastEditedBy && (
                        <div className="flex items-center justify-between border-t border-gray-200 shadow-sm pt-3">
                            <span className="text-sm text-gray-500">Last edited by</span>
                            <span className="text-white font-medium">{transaction.lastEditedBy.name}</span>
                        </div>
                    )}
                    {transaction.lastEditedAt && (
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-500">Last edited on</span>
                            <span className="text-white">{format(new Date(transaction.lastEditedAt), 'PPpp')}</span>
                        </div>
                    )}
                    {transaction.deletedBy && (
                        <div className="flex items-center justify-between border-t border-gray-200 shadow-sm pt-3">
                            <span className="text-sm text-gray-500">Deleted by</span>
                            <span className="text-red-400 font-medium">{transaction.deletedBy.name}</span>
                        </div>
                    )}
                    {transaction.deletedAt && (
                        <div className="flex items-center justify-between">
                            <span className="text-sm text-gray-500">Deleted on</span>
            ...
            {/* Actions */}
            <div className="pt-8 border-t border-gray-200 shadow-sm flex flex-col sm:flex-row gap-4">
              <button 
                onClick={downloadPDF}
              ...
              <button 
                onClick={downloadCSV}
                className="flex-1 bg-gray-100 text-gray-700 hover:bg-gray-200 font-semibold py-3 px-6 rounded-xl transition-colors flex items-center justify-center gap-2"
              >
