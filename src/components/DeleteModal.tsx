...
export function DeleteModal({ title, description, onConfirm, onCancel, loading = false }: DeleteModalProps) {
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-neutral-900 border border-neutral-800 rounded-2xl p-6 shadow-xl w-full max-w-sm relative animate-in fade-in zoom-in duration-200">
        ...
        <div className="flex gap-3 w-full">
            <button
               onClick={onCancel}
               disabled={loading}
               className="flex-1 bg-neutral-800 text-white font-medium py-2.5 rounded-xl hover:bg-neutral-700 transition disabled:opacity-50"
            >
               Cancel
            </button>
            <button
